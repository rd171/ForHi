...
CDriveIndex* _stdcall LoadIndexFromDisk(WCHAR *szPath)
{
	if(szPath)
        {
            wstring *pPath    = (wstring*)szPath;
            return new CDriveIndex(*pPath);
        }
	return NULL;
}
...
{
    TCHAR szVolumePath[_MAX_PATH] = {0};
    swprintf(szVolumePath, TEXT("\\\\.\\%c:"), cDriveLetter);
...
void CDriveIndex::PopulateIndex()
{
    ...
    swprintf(szRoot, TEXT("%c:\\"), m_cDrive);
